...
case "$1" in
    ...
    *)
        ...
        echo ""
        echo "Commands:"
        echo "  start     - Start the organizer daemon (PRODUCTION MODE)"
        echo "  stop      - Stop the organizer daemon"
        echo "  restart   - Restart the organizer daemon"
        echo "  status    - Check if organizer is running"
        echo "  log       - Tail the log file"
        echo "  test      - Run single scan (TEST MODE)"
        echo "  test-real - Run single scan (PRODUCTION MODE)"
        echo "  sync      - Synchronize folders only (PRODUCTION MODE)"
        echo "  dedupe    - Remove duplicates only (PRODUCTION MODE)"
        exit 1
        ;;
esac
