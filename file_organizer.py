...
class FolderSynchronizer:
    ...
    def __init__(self, logger: logging.Logger, config: Dict):
        self.logger = logger
        self.config = config
        self._rsync_supports_no_mmap = self._check_rsync_no_mmap_support()

    def _check_rsync_no_mmap_support(self) -> bool:
        """Return True if the installed rsync supports --no-mmap (requires rsync 3.x)."""
        try:
            result = subprocess.run(
                ['rsync', '--version'], capture_output=True, text=True, timeout=5
            )
            # First line is like: "rsync  version 3.2.7  protocol version 31"
            first_line = result.stdout.splitlines()[0] if result.stdout else ''
            import re
            m = re.search(r'version\s+(\d+)\.', first_line)
            if m and int(m.group(1)) >= 3:
                return True
            self.logger.info(
                f"rsync {first_line.strip()} does not support --no-mmap; "
                "install rsync 3.x (e.g. 'brew install rsync') to enable it"
            )
            return False
        except Exception:
            return False
    ...
    def sync_with_rsync(self, source: Path, target: Path, sync_mode: str, sync_excludes: List[str]) -> bool:
        ...
        try:
            ...
            # Disable mmap when configured, but only if this rsync version supports the flag.
            if self.config.get('rsync_disable_mmap', False) and self._rsync_supports_no_mmap:
                cmd.append('--no-mmap')
            ...
            try:
                ...
                if result.returncode != 0 and '--no-mmap' in cmd:
                    ...
                    if 'no-mmap' in stderr_lower and (
                        'unknown option' in stderr_lower or 'unrecognized option' in stderr_lower
                    ):
                        self.logger.warning("rsync does not support --no-mmap (older rsync version); retrying without it")
...
drives:
  MAIN_DRIVE: "/Users/janedoe"
  EXTERNAL_DRIVE: "/Volumes/ExternalDrive"
  PROTON_DRIVE: "MAIN_DRIVE/ProtonDrive"
  GOOGLE_DRIVE: "MAIN_DRIVE/GoogleDrive/MyFiles"
  LINUX_DRIVE: "/home/janedoe"
  WINDOWS_C: "C:/Users/janedoe"
  WINDOWS_D: "D:/"

# Two-way synchronization pairs - files copy in either direction.
sync_pairs:
  - folders:
...
# One-way backups (source -> target). Files only copy from A to B.
one_way_pairs:
  - folders:
      - "MAIN_DRIVE/Music"
      - "PROTON_DRIVE/Music"
...
# Folders matching these patterns are backed up to ~/organised (maintaining full source path structure)
# and replaced with soft links. This prevents their contents from being copied during sync operations.
...
# Files matching these patterns are excluded from sync. Note: ~/organised is always automatically excluded.
exclude_patterns:
  - ".DS_Store"
  - "*.pyc"
  - "*.log"
...
# ~/organized and ~/organised are different. See README.md for more details.
...
# enable_folder_sync: true = sync folders, false = only organize files
scan_interval: 500
...
def _write_default_config(config_file: str) -> None:
...
class EnhancedFileOrganizer:
    ...
    def __init__(self, config_file: str = "config.yaml", test_mode: bool = False):
        ...
        if not test_mode:
            ...
            # TEST MODE: config was already loaded and validated in _load_config()
            print(f"Found {self.config_file} - running in test mode with your config.")
        ...
        if test_mode:
            ...
            # Disable heavy ML models in test mode (sentence-transformers take minutes to load)
            self.config['enable_semantic_categories'] = False
            self.config['ml_content_analysis']['use_clip'] = False
    ...
    def _load_config(self) -> Dict:
        """Load configuration from YAML file, auto-generating a starter config if needed."""
        sep = "=" * 70

        def _next_steps() -> None:
            print(f"  1. Open {self.config_file} in a text editor.")
            print("  2. Set MAIN_DRIVE to your actual home directory,")
            print("     e.g. '/Users/yourname' on macOS, '/home/yourname' on Linux,")
            print("     or 'C:/Users/yourname' on Windows.")
            print("  3. Remove or update drives that don't apply to your setup.")
            print("  4. Adjust sync_pairs to the folders you want to keep in sync.")
            print("  5. Run the program again.")

        def _regen_and_exit(reason: str) -> None:
            """Write the default config and exit with a helpful message."""
            _write_default_config(self.config_file)
            print(f"\n{sep}")
            print("CONFIG FILE REGENERATED")
            print(sep)
            print(f"\n{reason}")
            print(f"\nA fresh '{self.config_file}' has been created.\n")
            _next_steps()
            print(f"\n{sep}\n")
            sys.exit(0)

        def _main_drive_error_and_exit(main_path: str, generated: bool) -> None:
            """Report a missing MAIN_DRIVE path and exit (without regenerating)."""
            print(f"\n{sep}")
            if generated:
                print("CONFIG FILE CREATED — PLEASE EDIT BEFORE RUNNING AGAIN")
            else:
                print("MAIN_DRIVE PATH IS INVALID")
            print(sep)
            if generated:
                print(f"\nNo '{self.config_file}' was found, so a starter file has been created.")
            print(f"\nMAIN_DRIVE is set to '{main_path}' which does not exist on this system.")
            print(f"\nNext steps:")
            _next_steps()
            print(f"\n{sep}\n")
            sys.exit(0)

        # ── 1. If config is missing, generate one ─────────────────────────────
        generated = False
        if not os.path.exists(self.config_file):
            ...
            # ── 2. Read and check existing config ────────────────────────────
            try:
                with open(self.config_file, 'r') as f:
                    content = f.read()
            except Exception as e:
                _regen_and_exit(f"Could not read '{self.config_file}': {e}")

            for line_num, line in enumerate(content.split('\n'), 1):
                if '\t' in line:
                    _regen_and_exit(
                        f"'{self.config_file}' contained TAB characters on line {line_num}.\n"
            ...
            try:
                ...
                _regen_and_exit(f"'{self.config_file}' contained invalid YAML:\n  {e}")

            if config is None:
                _regen_and_exit(f"'{self.config_file}' was empty or contained no valid YAML.")

            # Structural sanity (sync_pairs format, etc.) — regenerates on failure
            try:
                self._validate_config_sanity(config)
            except ValueError as e:
                _regen_and_exit(str(e))

        # ── 3. Parse freshly generated content (if we just wrote it) ─────────
        if generated:
            config = yaml.safe_load(content)

        # ── 4. MAIN_DRIVE must exist on disk ─────────────────────────────────
        drives = config.get('drives', {}) if isinstance(config, dict) else {}
        main_path = drives.get('MAIN_DRIVE') if isinstance(drives, dict) else None
        if isinstance(main_path, str):
            if not os.path.isdir(os.path.expanduser(main_path)):
                _main_drive_error_and_exit(main_path, generated)

        if generated:
            # MAIN_DRIVE exists (user really is /Users/janedoe on Mac) — still exit so
            # they review the rest of the config before the first real run.
            print(f"\n{sep}")
            print("CONFIG FILE CREATED")
            print(sep)
            print(f"\nA starter '{self.config_file}' has been created.")
            print("\nPlease review it and run the program again.")
            print(f"\n{sep}\n")
            sys.exit(0)
    ...
    def _validate_config_sanity(self, config: Dict):
        ...
        MAIN_DRIVE existence is checked in _load_config, not here.
        Other (optional) drives that are missing get a warning logged but are not fatal.
        """
        errors = []
        warnings = []
        ...
        # Print errors (always fatal — triggers config regeneration in _load_config)
        if errors:
            raise ValueError(
...
def validate_config_file(config_file: str) -> bool:
    ...
    If the file is missing or invalid, auto-generate a starter config, report the
    MAIN_DRIVE issue, and exit. Returns True if the existing config is valid.
    """
    sep = "=" * 70

    def _next_steps() -> None:
        print(f"  1. Open {config_file} in a text editor.")
        print("  2. Set MAIN_DRIVE to your actual home directory,")
        print("     e.g. '/Users/yourname' on macOS, '/home/yourname' on Linux,")
        print("     or 'C:/Users/yourname' on Windows.")
        print("  3. Remove or update drives that don't apply to your setup.")
    ...
    def _regen_and_exit(reason: str) -> None:
        ...
        print(sep)
        print(f"\n{reason}")
        print(f"\nA fresh '{config_file}' has been created.\n")
        _next_steps()
        print(f"\n{sep}\n")
        sys.exit(0)

    def _check_main_drive(config: dict, generated: bool) -> None:
        drives = config.get('drives', {})
        main_path = drives.get('MAIN_DRIVE') if isinstance(drives, dict) else None
        if isinstance(main_path, str) and not os.path.isdir(os.path.expanduser(main_path)):
            print(f"\n{sep}")
            if generated:
                print("CONFIG FILE CREATED — PLEASE EDIT BEFORE RUNNING AGAIN")
            else:
            ...
            print(sep)
            if generated:
                print(f"\nNo '{config_file}' was found, so a starter file has been created.")
            print(f"\nMAIN_DRIVE is set to '{main_path}' which does not exist on this system.")
            print(f"\nNext steps:")
            _next_steps()
            print(f"\n{sep}\n")
    ...
    # ── 1. Missing config — generate and report ───────────────────────────────
    if not os.path.exists(config_file):
        _write_default_config(config_file)
        cfg = yaml.safe_load(DEFAULT_CONFIG_CONTENT)
        _check_main_drive(cfg, generated=True)
        # MAIN_DRIVE exists (user really is /Users/janedoe) — still stop for review
        print(f"\n{sep}\nCONFIG FILE CREATED\n{sep}")
        print(f"\nA starter '{config_file}' has been created. Please review it and run again.\n")
        print(f"{sep}\n")
        sys.exit(0)
    ...
    try:
        ...
        _regen_and_exit(f"Could not read '{config_file}': {e}")

    for line_num, line in enumerate(content.split('\n'), 1):
        if '\t' in line:
            _regen_and_exit(
                f"'{config_file}' contained TAB characters on line {line_num}.\n"
                "YAML requires spaces, not tabs, for indentation."
            )
    ...
    try:
        ...
        _regen_and_exit(f"'{config_file}' contained invalid YAML:\n  {e}")

    if config is None:
        _regen_and_exit(f"'{config_file}' was empty or contained no valid YAML.")

    if not isinstance(config, dict):
        _regen_and_exit(f"'{config_file}' does not contain a valid configuration dictionary.")

    # ── 3. Check MAIN_DRIVE ───────────────────────────────────────────────────
    _check_main_drive(config, generated=False)

    # ── 4. Warn about optional drives that don't exist ────────────────────────
    drives = config.get('drives', {})
    if isinstance(drives, dict):
        missing = []
        for drive_name, drive_path in drives.items():
            if drive_name == 'MAIN_DRIVE':
                continue
            if not isinstance(drive_path, str):
                continue
            # Skip paths that reference other drive placeholders (resolved later)
            if any(other in drive_path for other in drives if other != drive_name):
                continue
            if not os.path.isdir(os.path.expanduser(drive_path)):
                missing.append(f"  • {drive_name} ('{drive_path}') does not exist — will be ignored")
        if missing:
            print(f"Configuration warnings in {config_file}:")
            for m in missing:
                print(m)
            print()
